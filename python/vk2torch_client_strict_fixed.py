...
class CUDADriverAPI:
    ...
    def memcpy_host_to_device_bytes(self, dst: ctypes.c_void_p, data: bytes):
        """Copy bytes from host to device memory."""
        size = len(data)
        # Create a ctypes buffer from bytes
        buf = (ctypes.c_ubyte * size).from_buffer_copy(data)
        result = self.cuda.cuMemcpyHtoD_v2(dst, ctypes.cast(buf, ctypes.c_void_p), size)
        if result != CUDA_SUCCESS:
            raise RuntimeError(f"cuMemcpyHtoD failed: {result}")

class VK2TorchClientStrictFixed:
    ...
    def __init__(self, socket_path: str = "/tmp/vk2torch.sock"):
        ...
        # STRICT MODE: Initialize CUDA as mandatory
    ...
    def update_camera_raw(self, camera_data: bytes) -> bool:
        """Update camera parameters using raw FrameConstants data."""
        if not self.connected or not self.cuda_api:
            logger.error("STRICT MODE ERROR: Not connected or CUDA not available")
            return False
        
        try:
            # Copy raw data directly to device buffer
            self.cuda_api.memcpy_host_to_device_bytes(self.dev_cam, camera_data)
            ...
            logger.error(f"STRICT MODE ERROR: Raw camera update failed: {e}")
            return False
    ...
    def get_frame(self, timeout_ms: int = 5000) -> 'torch.Tensor':
        ...
        if not HAS_CUPY or not HAS_TORCH:
            logger.error("STRICT MODE ERROR: CuPy or PyTorch not available - this should never happen")
            raise RuntimeError("STRICT MODE: CuPy/PyTorch missing - internal error")
