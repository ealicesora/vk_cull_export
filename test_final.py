...
def test_system():
    """Complete system test with integrated shared memory camera control."""
    
    # Test with integrated shared memory camera matrices
    from vk2torch_client import VK2TorchClient
    import numpy as np
    
    print("\nConnecting with VK2TorchClient (integrated shared memory camera protocol)...")
    ...
    if client.connect():
        ...
        # Test with varying camera positions using integrated shared memory
        print("\nTesting frame capture with different camera positions via integrated shared memory...")
        ...
        for i in range(0, 1000):  
            ...
            T = np.array([-2.80552141, -1.27673587,  3.06543639 + i * 0.0], dtype=np.float32)
            ...
            # Use integrated shared memory for camera matrices
            ...
            #     client.save_depth_png(frame, f"success_test{i}.png")
